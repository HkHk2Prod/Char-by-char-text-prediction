...
class Callback:
    def on_train_start(self, trainer: "Trainer") -> None: pass
    def on_epoch_start(self, epoch: int, trainer: "Trainer") -> None: pass
    def on_batch_end(self, epoch: int, step: int, loss: float, trainer: "Trainer") -> None: pass
    def on_epoch_end(self, epoch: int, train_metrics: dict, trainer: "Trainer") -> None: pass
    def on_val_end(self, epoch: int, val_metrics: dict, trainer: "Trainer") -> None: pass
...
class ConfigSaverCallback(Callback):
    """Saves trainer and model config to config.json at the start of training."""
...
class BatchLogCallback(Callback):
    ...
    def on_batch_end(self, epoch: int, step: int, loss: float, trainer: "Trainer") -> None:
        if step % self.log_every != 0:
            return
        print(f"  step {step:5d}/{len(trainer.train_loader)}"
...
class EvalCallback(Callback):
    ...
    def on_epoch_end(self, epoch: int, train_metrics: dict, trainer: "Trainer") -> None:
        ...
        t0          = time.time()
        val_metrics = _run_eval(trainer.model, self.val_loader, trainer.criterion, trainer.device)
...
class LogCallback(Callback):
    ...
    def on_epoch_end(self, epoch: int, train_metrics: dict, trainer: "Trainer") -> None:
        ...
        val_str = (f"  val loss {val_m['loss']:.4f}  val ppl {perplexity(val_m['loss']):7.2f}"
                   if val_m is not None else "")
        print(f"  [summary] train loss {train_metrics['loss']:.4f}"
...
class CheckpointCallback(Callback):
    ...
    def on_train_end(self, trainer: "Trainer") -> None:
        print(f"[ckpt] best val loss {self._best:.4f}, ppl {perplexity(self._best):.2f}, acc {self._best_accuracy:.4f}.")
...
class TestEvalCallback(Callback):
    ...
    def on_train_end(self, trainer: "Trainer") -> None:
        ...
        metrics = _run_eval(trainer.model, self.test_loader, trainer.criterion, trainer.device)
